...
class Bender(Tools):
    ...
    def magic(payload):
        ...
        if diff < 2 and diff > 0 and options["force"] == False:
            ...
            return {"lbMap":lbMap,"success":False,"possible":True,"line":line,"subnet":subnet,"msg":f"Direct route is better, keeping {line['ip_dst']} Lowest we got {float(latency[0][0])}ms vs {int(direct)}ms direct"}
        ...
        for row in self.files['pmacct_avg.json'].split('\n'):
            ...
            if len(threads) <= self.files['config.json']['threads']:
                ...
                if self.files['config.json']['lazy']:
                    ...
                    self.files['history.json'][options['subnet']] = {'ip':line['ip_dst'],'port':line['port_dst'],'bytes':line['bytes'],'lastBytes':line['bytes'],'rounds':0,'expiry':int(datetime.now().timestamp())}
